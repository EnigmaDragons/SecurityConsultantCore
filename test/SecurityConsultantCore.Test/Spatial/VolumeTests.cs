...
namespace SecurityConsultantCore.Test.Spatial
{
    ...
    public class VolumeTests
    {
        ...
        {
            var volume = new Volume(new List<XYVolume> { new XYVolume(0, 0), new XYVolume(1, 1) });

            var spaces = volume.GetOccupiedTiles(new XYZOrientation(0, 0, 0, Orientation.Default)).ToList();
        ...
        public void Volume_GetRotatedOccupiedSpaces_SpacesCorrect()
        {
            ...
            var spaces = volume.GetOccupiedTiles(new XYZOrientation(0, 0, 0, Orientation.Ninety)).ToList();
